...
def importar_basta_sin_fase(ruta_archivo):
    # Carga las 2 columnas omitiendo la primera línea (encabezado)
    datos = np.loadtxt(ruta_archivo, skiprows=1)
    
    # Ordena por la primera columna (frecuencia)
    datos = datos[datos[:, 0].argsort()]
    
    frecuencia = datos[:, 0]
    magnitud = datos[:, 1]
    
    return frecuencia, magnitud
...
def graficar_respuesta_db_solo_magnitud(frec, mag_db, titulo="Respuesta en Frecuencia"):
    fig, ax = plt.subplots(figsize=(8, 4.5))
    fig.suptitle(titulo, fontsize=12, fontweight="bold")

    # --- GRÁFICO DE MAGNITUD ---
    ax.semilogx(frec, mag_db, color='#1f77b4', label="Nivel de Presión Sonora (SPL)")
    ax.set_ylabel("Magnitud [dB]")
    ax.grid(True, which="both", linestyle="--", alpha=0.6)

    # Escala Y enfocada en la zona de interés hasta ka=1 (994 Hz)
    frec_arr = np.array(frec)
    mag_db_arr = np.array(mag_db)
    mascara_res = (frec_arr >= 20) & (frec_arr <= 994)
    if np.any(mascara_res):
        mag_filtrada = mag_db_arr[mascara_res]
        margen = (np.max(mag_filtrada) - np.min(mag_filtrada)) * 0.08
        ax.set_ylim(np.min(mag_filtrada) - margen, np.max(mag_filtrada) + margen)

    # Formato de ejes, ticks y línea ka=1
    aplicar_formato_ejes(ax)

    plt.tight_layout()
    plt.show()
    

def graficar_respuesta_excursion(frec, mag_db, titulo="Excursión"):
    fig, ax = plt.subplots(figsize=(8, 4.5))
    fig.suptitle(titulo, fontsize=12, fontweight="bold")

    # --- GRÁFICO DE MAGNITUD ---
    ax.semilogx(frec, mag_db, color='r', label="Nivel de Presión Sonora (SPL)")
    ax.set_ylabel("Excursion del cono [m]")
    ax.grid(True, which="both", linestyle="--", alpha=0.6)
...
def graficar_respuesta_velocidad(frec, mag_db, titulo="Velocidad"):
    fig, ax = plt.subplots(figsize=(8, 4.5))
    fig.suptitle(titulo, fontsize=12, fontweight="bold")

    # --- GRÁFICO DE MAGNITUD ---
    ax.semilogx(frec, mag_db, color='orange', label="Nivel de Presión Sonora (SPL)")
    ax.set_ylabel("Velocidad del vent [m/s]")
    ax.grid(True, which="both", linestyle="--", alpha=0.6)
...
frec_MOL_MODIFICADO, mag_MOL_MODIFICADO = importar_basta_sin_fase(r"exports/MOL_MODIFICADO.txt") 
frec_CONO_EXCURSION_MOL_MODIFICADO, mag_CONO_EXCURSION_MOL_MODIFICADO = importar_basta_sin_fase(r"exports/CONO_EXCURSION_MOL_MODIFICADO.txt") 
frec_VENT_VELOCITY_MOL_MODIFICADO, mag_VENT_VELOCITY_MOL_MODIFICADO = importar_basta_sin_fase(r"exports/VENT_VELOCITY_MOL_MODIFICADO.txt") 
...
graficar_respuesta_db_solo_magnitud(frec_MOL_MODIFICADO, mag_MOL_MODIFICADO)
graficar_respuesta_excursion(frec_CONO_EXCURSION_MOL_MODIFICADO, mag_CONO_EXCURSION_MOL_MODIFICADO)
graficar_respuesta_velocidad(frec_VENT_VELOCITY_MOL_MODIFICADO, mag_VENT_VELOCITY_MOL_MODIFICADO)